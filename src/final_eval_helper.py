...
#     out = metrics.evaluate(model, X, L_hat, y, masks=masks, device=device, need_aux=True)
#     final = {
...
#     if sigmas:
#         final["robust_feature_noise"] = metrics.feature_noise_eval(
#             model, X, L_hat, y, masks={"test": masks["test"]}, sigmas=sigmas, device=device
#         )
#     ps = parse_float_list(args.robust_edge_drop)
#     if ps:
#         L_sym = ds_meta.get("L_sym", None)
#         if L_sym is not None:
#             final["robust_edge_drop"] = metrics.edge_dropout_eval(
...
#     # tables CSV
#     import csv
#     class_names = ds_meta.get("class_names", [str(i) for i in range(int(y.max().item() + 1))])
#     with open(tables_dir / "predictions_test.csv", "w", newline="") as f:
#         w = csv.writer(f)
#         w.writerow(["node_id", "y_true", "y_pred", "confidence"])
#         idxs = torch.arange(y.size(0))[test_mask].cpu().numpy()
...
#     try:
#         aux_layers = out.get("aux", {}).get("layers", [])
